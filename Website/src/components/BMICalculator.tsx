...
const BMICalculator: React.FC<BMICalculatorProps> = ({ onBack }) => {
  ...
  const getBMICategory = (bmi: number) => {
    return BMI_CATEGORIES.find(cat => bmi >= cat.min && bmi < cat.max) || BMI_CATEGORIES[0];
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white p-4 md:p-8">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-bold">BMI Calculator & Analysis</h1>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          ...
          <div className="backdrop-blur-lg bg-white/5 rounded-2xl p-6 border border-gray-700">
            ...
            {!bmiResult ? (
              ...
              <div className="space-y-6">
                {/* BMI Score */}
                <div className="p-6 bg-gradient-to-r from-blue-500/20 to-indigo-500/20 rounded-xl border border-blue-500/20">
                ...
                <div className="flex-1 p-4 bg-white/5 rounded-xl">
                  ...
                  <div className="space-y-3 text-gray-200">
                    {bmiAnalysis
                      ...
                      .filter(tip => tip.trim())
                      .map((tip, index) => {
                        // Remove any remaining asterisks from the tip
                        const cleanTip = tip.replace(/\*\*/g, '').trim();
                        const [title, content] = cleanTip.split(':').map(s => s.trim());
                        if (!title || !content) return null;
                        
                        // Extract the base title without "recommendation" or "suggestion"
